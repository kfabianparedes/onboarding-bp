...
import { USERNAME_LOGIN_ERROR_MESSAGES } from "../../parameters/error-message";
...
export class UsernameSigninControl extends FormControl {
  ...
  public get usernameMessageError(): string {
    return getErrorbyFormControl(this, USERNAME_LOGIN_ERROR_MESSAGES);
  }
  ...
  private validateUsername(username: string): ReturnType<AsyncValidatorFn> {
    return timer(ASYNC_VALIDATION_DELAY).pipe(
      ...
      map( (existUsername:{ exists: boolean } ) => ( !existUsername.exists ? { usernameNotFound: true } : null)),
    );
  }
